...
import { createUserCreditService, UserCreditService } from "../../src/services/user/user-credit.service";
...
describe.sequential("Auth API", () => {
  ...
  // })
  it("should get user credit", async () => {
    userCreditService = createUserCreditService(env?.client!);
    const res = await userCreditService?.getUserCredit({
      userCredit: {
        userId: env?.userId
      },
    });
    ...
    expect(res).not.toBeNull();
    expect(res?.userCredit.id).not.toBeNull();
    expect(res?.userCredit.amount).greaterThan(0);
  })
